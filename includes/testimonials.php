<section class="testimonial pt-4">
    <div class="container">
        <div class="section-title mb-6 text-center w-75 mx-auto">
            <h2 class="mb-3">What Our Customers Say</h2>
        </div>
        <div class="row align-items-center">

            <div class="col-lg-7 ps-4">
                <div class="row review-slider">
                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057689264-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img1.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057689264-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">James G</h5>
                                        <span>Flitwick, United Kingdom</span>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057689264-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>We were waking happy each day!</b></p>
                                    <p class="mb-2">Prabash is the best driver and guide we have ever had, and we are widely travelled. He is unconceited but highly educated, empathetic and kind. He has a profound knowledge of the country and its culture.</p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057683082-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img2.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057683082-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">C S</h5>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r1057683082-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>Safety, Friendly and unforgettable service</b></p>
                                    <p class="mb-2">We traveled to Sri Lanka from Finland for an eight-day holiday and chose Ceylon Travel Lanka for our transportation. It was one of the best decisions we made for our trip.</p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r913140504-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img3.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r913140504-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">Bruce Y</h5>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r913140504-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>We started from Colombo, by Kandy, through Nuwara Eliya, droped by Kand and reched Palatupana for animal safari.</b></p>
                                    <p class="mb-2">The tour was so amazing.....We had an impressive experience with everything.....especially the people in Sri Lanka</p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891238576-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img4.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891238576-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">arnavsinha</h5>
                                        <span>Mohali, India</span>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891238576-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>Best travel experience ever!</b></p>
                                    <p class="mb-2">Prabash was our driver and guide during my trip to Sri Lanka with my parents. Throughout our trip, lasting nearly 10 days, he ensured all our needs were taken care of. He knew the roads well, helped out with sightseeing, and knew where to stop for shopping and food. </p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891136140-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img5.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891136140-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">Boniface8th</h5>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891136140-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>Book Prabash at once!</b></p>
                                    <p class="mb-2">We were lucky enough to have Prabash for a ten day trip. Not only is he fantastically knowledgeable, his kindness and consideration was wonderful. He took such care and gave such good advice about what was possible for my parents, who are in the late 70s.</p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>


                    <div class="col-sm-4 item">
                        <div class="testimonial-item1 rounded">
                            <div class="author-info d-flex align-items-center mb-4">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891208846-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <img src="<?= BASE_URL ?>images/testimonial/img6.jpg" alt>
                                </a>
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891208846-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <div class="author-title ms-3">
                                        <h5 class="m-0 theme">Boniface8th</h5>
                                    </div>
                                </a>
                            </div>
                            <div class="details">
                                <a href="https://www.tripadvisor.com/ShowUserReviews-g11999643-d25571146-r891208846-Ceylon_Travel_Lanka-Minuwangoda_Western_Province.html" target="_blank">
                                    <p class="mb-2"><b>Thanks Prabash..!</b></p>
                                    <p class="mb-2">We are a couple from Australia, and we can describe our experience in one simple word, AMAZING!!! Our Tourguide Prabash is so cool, helpfull, always available and Prabash is one of the best guides that we have had so far. The way that he managed all the trips, situations, suggestions that he provided to us were amazing.</p>
                                    <p class="m-0">Read More →</p>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-5 pe-4">
                <div class="testimonial-image">
                    <img src="<?= BASE_URL ?>images/travel2.webp" alt>
                </div>
            </div>
        </div>
    </div>
</section>